...
bool MainWidget::editInstanceValues(InstanceMetadata& inst)
{
    ...
    for (const auto& f : leafs) {
        ...
        bool ok = false;
        QString v = QInputDialog::getText(this, QString::fromUtf8(u8"编辑实例值"), f + QString::fromUtf8(u8" 的值"), QLineEdit::Normal, cur, &ok);
        if (!ok) continue;
        inst.values[f] = v;
    }
    return true;
...
void MainWidget::onInstancesContextMenuRequested(const QPoint& pos)
{
    ...
    if (chosen == actAdd) {
        ...
        bool ok = false;
        QString typeName = QInputDialog::getItem(this, QString::fromUtf8(u8"选择类型"), QString::fromUtf8(u8"从模板选择结构体"), types, 0, false, &ok);
        if (!ok || typeName.isEmpty()) return;
        bool ok2 = false;
        QString instName = QInputDialog::getText(this, QString::fromUtf8(u8"新建实例"), QString::fromUtf8(u8"实例名称"), QLineEdit::Normal, "NewInstance", &ok2);
        ...
        bool ok = false;
        QString newName = QInputDialog::getText(this, QString::fromUtf8(u8"重命名"), QString::fromUtf8(u8"新名称"), QLineEdit::Normal, item->text(0), &ok);
